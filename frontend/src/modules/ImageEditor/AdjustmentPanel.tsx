...
export default function AdjustmentPanel({ imageState, onUpdate }: AdjustmentPanelProps) {
  const sliders = [
      ...
      {sliders.map(({ key, label, min, max }) => (
        <div key={key} className="space-y-1.5">
          <div className="flex justify-between text-xs">
            ...
            <span className="font-mono text-foreground">{imageState[key] > 0 ? '+' : ''}{imageState[key]}</span>
          </div>
          <Slider
            min={min}
            max={max}
