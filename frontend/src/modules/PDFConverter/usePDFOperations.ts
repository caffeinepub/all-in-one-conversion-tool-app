...
async function renderPDFPageToThumbnail(file: File, pageIndex: number): Promise<string> {
...
export function usePDFOperations(): UsePDFOperationsReturn {
  ...
  const convertToDownloadable = useCallback(async (selectedIds?: string[]) => {
    setIsConverting(true);
    try {
      const PDFLib = await loadPDFLib();
      const { PDFDocument } = PDFLib;

      const filesToConvert = selectedIds && selectedIds.length > 0
  ...
  const reorderMergeSplitEntries = useCallback((fromIndex: number, toIndex: number) => {
    setMergeSplitEntries(prev => {
      const next = [...prev];
  ...
  const mergeAndDownload = useCallback(async (selectedIds?: string[]) => {
    setIsMerging(true);
    try {
      const PDFLib = await loadPDFLib();
      const { PDFDocument } = PDFLib;
      ...
      if (selectedIds && selectedIds.length > 0) {
        entriesToMerge = entriesToMerge.filter(e => selectedIds.includes(e.id));
      }
