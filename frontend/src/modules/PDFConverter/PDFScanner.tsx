...
import { Upload, FileImage, Trash2, Download, ScanLine, Sun, Contrast, Type, Camera } from 'lucide-react';
...
function applyAdjustments(
  ...
  if (darkenText) {
    ...
    for (let i = 0; i < d.length; i += 4) {
      ...
      if (lum < 128) { d[i] = 0; d[i + 1] = 0; d[i + 2] = 0; }
...
function CropOverlay({ imageUrl, initialCrop, onApply, onCancel }: CropOverlayProps) {
  const imgRef = useRef<HTMLImageElement>(null);
  const containerRef = useRef<HTMLDivElement>(null);
  const [naturalSize, setNaturalSize] = useState<{ w: number; h: number } | null>(null);
  const [displaySize, setDisplaySize] = useState<{ w: number; h: number } | null>(null);
  const [crop, setCrop] = useState<CropRect | null>(null);
  const dragging = useRef<{ handle: DragHandle['corner']; startX: number; startY: number; startCrop: CropRect } | null>(null);
  ...
  const toDisplay = (v: number, axis: 'x' | 'y') => {
    ...
    return axis === 'x' ? (v / naturalSize.w) * displaySize.w : (v / naturalSize.h) * displaySize.h;
  };

  const toNatural = (v: number, axis: 'x' | 'y') => {
    if (!naturalSize || !displaySize) return v;
    return axis === 'x' ? (v / displaySize.w) * naturalSize.w : (v / displaySize.h) * naturalSize.h;
  };

  const onMouseDown = (e: React.MouseEvent, corner: DragHandle['corner']) => {
    e.preventDefault();
    if (!crop) return;
    dragging.current = { handle: corner, startX: e.clientX, startY: e.clientY, startCrop: { ...crop } };
  ...
  const onTouchStart = (e: React.TouchEvent, corner: DragHandle['corner']) => {
    ...
    if (!crop) return;
    const t = e.touches[0];
    dragging.current = { handle: corner, startX: t.clientX, startY: t.clientY, startCrop: { ...crop } };
  ...
  const applyDrag = (clientX: number, clientY: number) => {
    ...
    if (handle === 'tl') {
      ...
      const nx = Math.min(x + w - minSize, x + dx);
      w = w - (nx - x); x = nx; h = Math.max(minSize, h + dy);
    } else {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/80">
      <div className="flex flex-col items-center gap-4 p-4 max-w-2xl w-full">
        ...
        <div ref={containerRef} className="relative inline-block">
          ...
          {crop && displaySize && naturalSize && handles.map(corner => {
            const cx = corner.includes('r') ? toDisplay(crop.x + crop.w, 'x') : toDisplay(crop.x, 'x');
            const cy = corner.includes('b') ? toDisplay(crop.y + crop.h, 'y') : toDisplay(crop.y, 'y');
            return (
              <div
                key={corner}
        ...
        <div className="flex gap-3">
          <Button onClick={() => crop && onApply(crop)} size="sm">Apply Crop</Button>
...
export default function PDFScanner() {
    ...
    reader.onload = ev => {
      ...
      img.onload = () => {
        ...
        setScannedPages(prev => [...prev, {
          id, dataUrl, brightness: 100, contrast: 100, darkenText: false, cropRect: crop
        }]);
  ...
  const updatePage = (id: string, patch: Partial<ScannedPage>) => {
    setScannedPages(prev => prev.map(p => p.id === id ? { ...p, ...patch } : p));
  };
  ...
  const convertToPDF = useCallback(async () => {
    ...
    try {
      ...
      for (const page of scannedPages) {
        const img = new Image();
        await new Promise<void>(res => { img.onload = () => res(); img.src = page.dataUrl; });
        const adjusted = applyAdjustments(img, page.brightness, page.contrast, page.darkenText, page.cropRect);
      ...
      const blob = new Blob([pdfBytes.slice(0).buffer as ArrayBuffer], { type: 'application/pdf' });
      setPdfBlob(blob);
    } catch (err) {
    ...
    <div className="flex flex-col gap-6">
      ...
      <div className="glass-card p-6 flex flex-col items-center gap-4">
        ...
        <p className="text-sm text-muted-foreground text-center">
          Use your device camera to scan documents one by one, or import images from files.
        </p>
        ...
        <p className="text-xs text-muted-foreground text-center">
          Tap <strong>Scan with Camera</strong> to capture documents one by one. Each photo is added as a new page.
        </p>
      </div>
              ...
              {scannedPages.map((page, idx) => (
                <div
                  ...
                  className={`relative shrink-0 cursor-pointer rounded-lg overflow-hidden border-2 transition-colors ${
                    selectedPageId === page.id ? 'border-primary' : 'border-transparent'
                  }`}
            ...
            <div className="glass-card p-4 flex flex-col gap-4 flex-1 min-w-0">
              <div className="flex items-center justify-between">
                <p className="text-sm font-medium">
                  Page {scannedPages.findIndex(p => p.id === selectedPage.id) + 1} Adjustments
                </p>
                <Button
                  variant="outline"
                  size="sm"
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <Slider
                    ...
                    onValueChange={([v]) => updatePage(selectedPage.id, { brightness: v })}
                    className="flex-1"
                  />
                </div>
              </div>
              ...
              <div className="flex flex-col gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <Slider
                    ...
                    onValueChange={([v]) => updatePage(selectedPage.id, { contrast: v })}
                    className="flex-1"
                  />
                </div>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <Label className="text-xs text-muted-foreground flex-1">Darken Text (B&W)</Label>
                <Switch
                  checked={selectedPage.darkenText}
                  onCheckedChange={v => updatePage(selectedPage.id, { darkenText: v })}
                />
              </div>
            </div>
          )}
        ...
        <div className="glass-card p-4 flex flex-col gap-4">
          <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
            <div className="flex-1 flex flex-col gap-1">
              <label className="text-xs text-muted-foreground font-medium">Output filename</label>
              <input
                type="text"
                value={filename}
          ...
          {pdfBlob && (
            <p className="text-xs text-green-600 dark:text-green-400 flex items-center gap-1">
              ...
              PDF ready — {(pdfBlob.size / 1024).toFixed(1)} KB · {scannedPages.length} page{scannedPages.length !== 1 ? 's' : ''}
            </p>
          )}
        </div>
      )}
