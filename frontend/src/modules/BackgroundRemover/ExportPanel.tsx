...
import { Download, ZoomIn, ZoomOut, RotateCcw, ImagePlus, Loader2, Archive } from 'lucide-react';
...
export default function ExportPanel({
  ...
  const handleDownload = useCallback(async () => {
    ...
    try {
      if (exportFormat === 'png') {
        ...
        await new Promise<void>(resolve => { img.onload = () => resolve(); img.src = resultDataUrl; });
  ...
  return (
    <div className="glass-card p-4 space-y-4">
      <h3 className="text-sm font-semibold text-foreground">Export &amp; Tools</h3>

      {/* Zoom Controls */}
      <div>
        <label className="section-title">Zoom: {Math.round(zoom * 100)}%</label>
      ...
      {/* Export Format */}
      <div>
        <label className="section-title">Download Format</label>
        ...
        {exportFormat === 'png' && (
          <p className="text-xs text-muted-foreground mt-1">PNG preserves transparency</p>
        )}
      </div>
