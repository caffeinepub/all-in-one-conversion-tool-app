...
export function useImageCanvas() {
  ...
  const renderCanvas = useCallback(() => {
    ...
    let srcX = 0;
    let srcY = 0;
    let srcW = originalImage.width;
    let srcH = originalImage.height;
    if (cropRect) {
      srcX = cropRect.x;
      srcY = cropRect.y;
      srcW = cropRect.w;
    ...
    for (const layer of textLayers) {
      ...
      const useShadow =
        layer.shadowEnabled !== undefined ? layer.shadowEnabled : layer.shadow;
      if (useShadow) {
        const shadowColor = layer.shadowColor || "rgba(0,0,0,0.7)";
        const shadowBlur =
          layer.shadowBlur !== undefined ? layer.shadowBlur : 6;
        const shadowOffsetX =
          layer.shadowOffsetX !== undefined ? layer.shadowOffsetX : 2;
        const shadowOffsetY =
          layer.shadowOffsetY !== undefined ? layer.shadowOffsetY : 2;
  ...
  const exportImage = useCallback(
    (format: string, quality: number): string | null => {
      const canvas = canvasRef.current;
