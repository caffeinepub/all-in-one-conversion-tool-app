...
async function convertImage(
  ...
  return new Promise((resolve, reject) => {
    ...
    img.onload = () => {
      ...
      const quality = ["jpeg", "webp"].includes(settings.format)
  ...
  const convertAll = useCallback(async () => {
    ...
    for (const image of images) {
      try {
        ...
        setError(
          `Failed to convert ${image.name}: ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    }
  ...
  const convertSingle = useCallback(
    async (imageFile: ImageFile) => {
      try {
        ...
        setError(
          `Failed to convert ${imageFile.name}: ${err instanceof Error ? err.message : "Unknown error"}`,
        );
      }
    },
    [settings],
