...
import type { MergeSplitEntry } from "./usePDFOperations";
...
export default function MergeSplitTools({
  ...
  const allSelected =
    activeEntries.length > 0 && selectedIds.size === activeEntries.length;
  const someSelected =
    selectedIds.size > 0 && selectedIds.size < activeEntries.length;
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2">
        <label className="cursor-pointer">
          ...
          />
          <span
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm font-medium border border-border bg-card hover:bg-primary/10 hover:border-primary/40 transition-colors ${isLoading ? "opacity-50 pointer-events-none" : ""}`}
          >
