...
export function PageManager({ pdfFiles, onRemovePages }: PageManagerProps) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: intentionally running when selectedFileId changes
  useEffect(() => {
      ...
      <div className="flex items-center gap-3">
        ...
        {selectedPages.length > 0 && (
          ...
          >
            ...
            Remove {selectedPages.length} page
      ...
      {!loadingThumbs && thumbs.length > 0 && (
        <div className="space-y-3">
          <p className="text-xs text-muted-foreground">
            {thumbs.length} page{thumbs.length !== 1 ? "s" : ""} · drag to
            reorder · check to select for removal
          </p>
