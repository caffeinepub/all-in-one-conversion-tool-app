...
import {
  ...
  useImageConversion,
} from "./ImageConverter/useImageConversion";

function PreviewImage({ src, alt }: { src: string | null; alt: string }) {
  const [error, setError] = React.useState(false);
  const [loaded, setLoaded] = React.useState(false);

  // biome-ignore lint/correctness/useExhaustiveDependencies: resetting state when src changes
  React.useEffect(() => {
    setError(false);
    setLoaded(false);
  }, [src]);
...
export function ImageConverter() {
  ...
  const handleSettingsChange = React.useCallback(
    (partial: Partial<ConversionSettingsType>) => {
      setSettings((prev) => ({ ...prev, ...partial }));
    },
    [setSettings],
  );

  const _goToTab = React.useCallback((tab: TabValue) => {
    setActiveTab(tab);
  }, []);
