...
export default function VideoCutter() {
  ...
  const loadFFmpeg = useCallback(async () => {
    ...
    try {
      ...
      setError(
        "Failed to load video processing library. Using fallback method.",
      );
    } finally {
  ...
  const handleCutAndDownload = async () => {
    ...
    try {
        ...
        if (!ffmpegRef.current) {
          ...
          ff.on("progress", (data: unknown) => {
            ...
            if (d.progress !== undefined)
              setProgress(Math.round(d.progress * 100));
          });
          await ff.load();
      ...
      try {
        ...
        setError(
          "Failed to cut video. Please try a shorter clip or a different browser.",
        );
      }
    } finally {
      setIsCutting(false);
  ...
  const fallbackCut = async (): Promise<void> => {
    return new Promise((resolve, reject) => {
      ...
      const drawFrame = () => {
        ...
        const prog =
          ((video.currentTime - startTime) / (endTime - startTime)) * 100;
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500">
              Import video, select a range, and download the clip
            </p>
          </div>
          {videoUrl && (
            <Button
          ...
          {!ffmpegLoaded && (
            ...
            >
              {ffmpegLoading ? (
                ...
                <>
                  <Play className="w-4 h-4 mr-2" /> Load FFmpeg for Better
                  Quality
                </>
              )}
            </Button>
