...
export default function CropOverlay({
  ...
  const getHandle = useCallback(
    (e: React.MouseEvent | React.TouchEvent): HandleType => {
      ...
      if (Math.abs(mx - (dx + dw)) < hs && Math.abs(my - (dy + dh)) < hs)
        return "br";
      if (Math.abs(mx - (dx + dw / 2)) < hs && Math.abs(my - dy) < hs)
        return "tm";
      if (Math.abs(mx - (dx + dw / 2)) < hs && Math.abs(my - (dy + dh)) < hs)
        return "bm";
      if (Math.abs(mx - dx) < hs && Math.abs(my - (dy + dh / 2)) < hs)
        return "ml";
      if (Math.abs(mx - (dx + dw)) < hs && Math.abs(my - (dy + dh / 2)) < hs)
        return "mr";
  ...
  const handleMouseMove = useCallback(
    ...
    // biome-ignore lint/correctness/useExhaustiveDependencies: toImage recreates on every render but is safe to include
